...
@ApplyOptions<Command.Options>({
  description: `Recieve your weekly amount of ${currencyName}.`,
  cooldownDelay: ms("7d")
})
export class UserCommand extends Command {
  ...
  private async respond(interactionOrMessage: Message | Command.ChatInputCommandInteraction) {
    ...
    await db.user.upsert({
      ...
      create: {
        id: user.id,
        wallet: rewards.weekly
      },
      update: {
        wallet: {
          increment: rewards.weekly
        }
      }
    });
    ...
    return reply({
      embeds: [new EmbedBuilder().setDescription(`You have recieved ${currency}${rewards.weekly.toLocaleString()}!`).setColor(colors.primary)]
    });
  }
}
